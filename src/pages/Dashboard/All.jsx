...
const All = () => {
    ...
    axios.post(`${URL}/deleteTodo`, todo)
      .then((res) => {
        ...
        if(res.data === "Todo Deleted"){
          let documentsAfterdelete = documents.filter(doc => doc._id !== todo._id)
  ...
  <tbody>
    {filteredDocuments.map((todo, i)=>{
      ...
      <td>{todo.status}</td>
      <td>
        <Button className='me-2' type='primary' size='small'  onClick={()=>{handleEdit(todo)}} >Edit</Button>
        <Button type='primary' danger size='small' onClick={()=>{handleDelete(todo)}} >Delete</Button>
        </td>
    </tr>
    )
    })
...
:<div className='d-flex justify-content-center'><span><Spin /></span></div>
      ...
      <div>
        <Row className='text-center py-2'>
            <Col span={24}>
             <Button type="primary"  onClick={()=>{navigate("/dashboard/add")}}>Add Todo</Button>
            </Col>
        </Row>
      </div>
    </main>
